...
Позволяет применять к пользовательскому `Действие` или лямбда-выражению стратегии повтора, предохранителя, ограничения параллелизма, таймаута, ограничения вызовов и запасного сценария в составе пайплайна.
...
Если операция завершается с ошибкой, метод `Получить()` выбрасывает `ИнформацияОбОшибке` из [async](https://github.com/sfaqer/async/).

Для обработки ошибки или преобразования результата используйте метод `Поймать()` у объекта `Обещание`.
Короткий пример см. в [руководстве по лямбда-выражениям](docs/ЛямбдаВыражения.md), а подробности о запуске — в [документации пайплайна](docs/ПайплайнОтказоустойчивости.md).
